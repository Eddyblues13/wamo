<x-admin-layout title="Edit deposit method">

    <a href="{{ route('admin.deposit-methods.index') }}" class="text-sm text-white/50 transition hover:text-white">← Back to methods</a>

    <form action="{{ route('admin.deposit-methods.update', $method) }}" method="post" class="mt-4 max-w-3xl rounded-3xl glass p-7">
        @csrf @method('PUT')
        <h2 class="mb-6 text-xl font-bold">Edit {{ $method->name }}</h2>
